...
exports.create = async (req, res) => {
    try {
        ...
        if (query && query.length > 0) {
            results = await commonService.insertMany(db.cityModel, query);
        } else {
            results = await commonService.insertOne(db.cityModel, query);
        }
...
exports.get = async (req, res) => {
    try {
        ...
        if (req.query && req.query.city_Id) {
            results = await commonService.findOne(db.cityModel, query);
        } else {
            results = await commonService.findAll(db.cityModel, query);
        }
