...
exports.create = async (req, res) => {
    try {
        ...
        if (query && query.length > 0) {
            results = await commonService.insertMany(db.facilityModel, query);
        } else {
            results = await commonService.insertOne(db.facilityModel, query);
        }
...
exports.get = async (req, res) => {
    try {
        ...
        if (req.query && req.query.city_Id) {
            results = await commonService.findOne(db.facilityModel, query);
        } else {
            results = await commonService.findAll(db.facilityModel, query);
        }
